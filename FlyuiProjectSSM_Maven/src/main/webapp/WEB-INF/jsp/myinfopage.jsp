		...
		<div style="width:1170px;margin:0 auto;height: 100px;">
				...
				<div style="width:538px;margin: 0 auto;">
				   ...
				   <c:forEach items="${myArticle }" var="article">
					<div style="width: 100%;border-bottom:1px solid #F2F2F2;height: 40px;line-height:40px ;">
						<a style="margin-right: 20px;" href="${pageContext.request.contextPath }/article/info/${article.id}">${article.title }</a><span>${article.releasetime }</span><span style="float: right;"><span>${article.visitnum }</span>阅/<span>${article.remarknum }</span>答</span>
					</div>
					</c:forEach>
			...
			<div style="width:578px;float: right;background: white;">
				...
				<c:forEach items="${ramarklist }" var="remarkArticle">
					<span>${remarkArticle.remarktime } 在 <a href="${pageContext.request.contextPath }/article/info/${remarkArticle.id}" style="color: dodgerblue;">${remarkArticle.title }</a> 中回答：</span>
